...
USE `FirstPlace`$$
DROP TRIGGER IF EXISTS `FirstPlace`.`Employee_Log_Trigger` $$
USE `FirstPlace`$$
...
FROM Log_Type
WHERE Log_Type.log_ty_name = 'Employee';
...
START TRANSACTION;
USE `FirstPlace`;
INSERT INTO `FirstPlace`.`Log_Type` (`log_ty_ID`, `log_ty_name`) VALUES (1, 'Employee');
INSERT INTO `FirstPlace`.`Log_Type` (`log_ty_ID`, `log_ty_name`) VALUES (2, 'Attendance');
