...
CREATE  OR REPLACE VIEW `FirstPlace`.`logEmployee_view` AS
 select 
    ...
    from
		...
		`log_Type` AS `loty`
    where
        ((`e`.`emp_id` = `lo`.`emp_id`) AND (`lo`.`log_type_id` = `loty`.`log_ty_id`));
...
USE `FirstPlace`$$
DROP TRIGGER IF EXISTS `FirstPlace`.`Employee_Log_Trigger_Add` $$
USE `FirstPlace`$$


CREATE TRIGGER `Employee_Log_Trigger` AFTER INSERT ON Employee FOR EACH ROW
-- Edit trigger body code below this line. Do not edit lines above this one
BEGIN
INSERT INTO Logs (`log_Date`, `emp_id`, `log_type_id`)
SELECT now(), new.emp_id, Log_Type.log_ty_id
FROM Log_Type
WHERE Log_Type.log_ty_name = 'Employee_Add';
...
USE `FirstPlace`$$
DROP TRIGGER IF EXISTS `FirstPlace`.`Employee_Log_Trigger_Update` $$
USE `FirstPlace`$$
...
SELECT now(), new.emp_id, Log_Type.log_ty_id
FROM Log_Type
WHERE Log_Type.log_ty_name = 'Employee_Update';
...
-- Edit trigger body code below this line. Do not edit lines above this one
BEGIN
INSERT INTO Logs (`log_Date`, `emp_id`, `log_type_id`, `att_id`)
SELECT now(), 1, Log_Type.log_ty_id, new.Student_s_id
FROM Log_Type
...
INSERT INTO `FirstPlace`.`Log_Type` (`log_ty_ID`, `log_ty_name`) VALUES (NULL, 'Attendence_Add');
INSERT INTO `FirstPlace`.`Log_Type` (`log_ty_ID`, `log_ty_name`) VALUES (NULL, 'Attendence_Update');
