...
-- -----------------------------------------------------
-- Table `FirstPlace`.`Employee_has_Role`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `FirstPlace`.`Employee_has_Role` ;

CREATE  TABLE IF NOT EXISTS `FirstPlace`.`Employee_has_Role` (
  `Employee_emp_id` INT NOT NULL ,
  `Role_Role_ID` INT NOT NULL ,
  PRIMARY KEY (`Employee_emp_id`, `Role_Role_ID`) ,
  INDEX `fk_Employee_has_Role_Role1_idx` (`Role_Role_ID` ASC) ,
  INDEX `fk_Employee_has_Role_Employee1_idx` (`Employee_emp_id` ASC) ,
...
-- -----------------------------------------------------
-- Table `FirstPlace`.`Class_has_Employee`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `FirstPlace`.`Class_has_Employee` ;

CREATE  TABLE IF NOT EXISTS `FirstPlace`.`Class_has_Employee` (
  `Class_c_id` INT NOT NULL ,
  `Employee_emp_id` INT NOT NULL ,
  PRIMARY KEY (`Class_c_id`, `Employee_emp_id`) ,
  INDEX `fk_Class_has_Employee_Employee1_idx` (`Employee_emp_id` ASC) ,
  INDEX `fk_Class_has_Employee_Class1_idx` (`Class_c_id` ASC) ,
  CONSTRAINT `fk_Class_has_Employee_Class1`
    FOREIGN KEY (`Class_c_id` )
    REFERENCES `FirstPlace`.`Class` (`c_id` )
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Class_has_Employee_Employee1`
    FOREIGN KEY (`Employee_emp_id` )
    REFERENCES `FirstPlace`.`Employee` (`emp_id` )
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
-- -----------------------------------------------------
-- Placeholder table for view `FirstPlace`.`Viewclass_grade_room_term_teacher`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `FirstPlace`.`Viewclass_grade_room_term_teacher` (`Grade` INT, `Room` INT, `SchoolYear` INT, `Teacher` INT);

-- -----------------------------------------------------
-- Placeholder table for view `FirstPlace`.`ViewStudentHasClass`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `FirstPlace`.`ViewStudentHasClass` (`Student_s_id` INT, `Class_c_id` INT);
...
-- -----------------------------------------------------
-- View `FirstPlace`.`Viewclass_grade_room_term_teacher`
-- -----------------------------------------------------
DROP VIEW IF EXISTS `FirstPlace`.`Viewclass_grade_room_term_teacher` ;
DROP TABLE IF EXISTS `FirstPlace`.`Viewclass_grade_room_term_teacher`;
USE `FirstPlace`;
CREATE  OR REPLACE VIEW `FirstPlace`.`Viewclass_grade_room_term_teacher` AS

select s.ClassName as Grade, r.Location as Room, sy.Name as SchoolYear, ro.RoleName as Teacher
from Class c, SchoolYear sy, StdClass s, Room r, student_has_class studClass, employee e, 
student stud, employee_has_role emp, role ro
where c.SchoolYear_sy_id = sy.sy_id and c.stdC_id = s.stdC_id and c.Room_Room_ID = r.Room_ID
and studClass.Class_c_id = c.c_id and studClass. Student_s_id = stud.s_id
and stud.Employee_emp_id = e.emp_id and e.emp_id = emp.Employee_emp_id
and emp.Role_Role_ID = ro.Role_ID and ro.RoleName = 'Teacher'
group by SchoolYear;
...
From StudentHasClass right outer join Class 
on Class_c_ID = c_ID;
