...
USE `FirstPlace`$$
DROP TRIGGER IF EXISTS `FirstPlace`.`Logs_AINS` $$
USE `FirstPlace`$$


CREATE TRIGGER `Logs_AINS` AFTER INSERT ON Logs FOR EACH ROW
-- Edit trigger body code below this line. Do not edit lines above this one
BEGIN
IF(new.new_att_Ty > 3) THEN
INSERT INTO Notification (`Notf_Date`,	`Student_s_id`, `Employee_emp_id`, `Att_Ty_id`)
SELECT now(), new.att_ID, s.Employee_emp_id, new.new_att_Ty
FROM Student AS s
WHERE new.att_ID = s.s_id;
END IF;
...
USE `FirstPlace`$$
DROP TRIGGER IF EXISTS `FirstPlace`.`Logs_AUPD` $$
USE `FirstPlace`$$


CREATE TRIGGER `Logs_AUPD` AFTER UPDATE ON Logs FOR EACH ROW
-- Edit trigger body code below this line. Do not edit lines above this one
BEGIN
IF(new.new_att_Ty > 3) THEN
INSERT INTO Notification (`Notf_Date`,	`Student_s_id`, `Employee_emp_id`, `Att_Ty_id`)
...
-- -----------------------------------------------------
-- Data for table `FirstPlace`.`Notification`
-- -----------------------------------------------------
START TRANSACTION;
USE `FirstPlace`;
INSERT INTO `FirstPlace`.`Notification` (`Notf_ID`, `Student_s_id`, `Employee_emp_id`, `Notf_Date`, `Att_Ty_id`) VALUES (NULL, 1, 3, '2013-06-05', 3);
INSERT INTO `FirstPlace`.`Notification` (`Notf_ID`, `Student_s_id`, `Employee_emp_id`, `Notf_Date`, `Att_Ty_id`) VALUES (NULL, 2, 3, '2013-06-05', 4);
